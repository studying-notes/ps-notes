...
function CreateProfileMenuItems(
	...
	if ($layout -eq "Default") {
		...
		# CreateMenuItem "Registry::HKEY_CURRENT_USER\SOFTWARE\Classes\Directory\shell\MenuTerminal_$guid"			"在 $name 中打开" $profileIcon $command $false
		CreateMenuItem "Registry::HKEY_CURRENT_USER\SOFTWARE\Classes\Directory\shell\MenuTerminalAdmin_$guid"			"在 $name 中打开" $profileIcon $elevated $true
		# CreateMenuItem "Registry::HKEY_CURRENT_USER\SOFTWARE\Classes\Directory\Background\shell\MenuTerminal_$guid"		"在 $name 中打开" $profileIcon $command $false
		CreateMenuItem "Registry::HKEY_CURRENT_USER\SOFTWARE\Classes\Directory\Background\shell\MenuTerminalAdmin_$guid"	"在 $name 中打开" $profileIcon $elevated $true
	}
}
...
function CreateMenuItems(
	...
	if ($layout -eq "Default") {
		...
		New-ItemProperty -Path 'Registry::HKEY_CURRENT_USER\SOFTWARE\Classes\Directory\shell\MenuTerminalAdmin'			-Name 'MUIVerb'				-PropertyType String -Value '在 Windows Terminal 中打开' | Out-Null
		...
		New-ItemProperty -Path 'Registry::HKEY_CURRENT_USER\SOFTWARE\Classes\Directory\Background\shell\MenuTerminalAdmin'	-Name 'MUIVerb'				-PropertyType String -Value '在 Windows Terminal 中打开' | Out-Null
		...
		CreateMenuItem "Registry::HKEY_CURRENT_USER\SOFTWARE\Classes\Directory\shell\MenuTerminalMini_Admin"			"在 Windows Terminal 中打开"	$icon $elevated $true
		...
		CreateMenuItem "Registry::HKEY_CURRENT_USER\SOFTWARE\Classes\Directory\Background\shell\MenuTerminalMini_Admin"		"在 Windows Terminal 中打开"	$icon $elevated $true
		return
	}
